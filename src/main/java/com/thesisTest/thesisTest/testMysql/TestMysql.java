...
public class TestMysql implements CommandLineRunner {
    ...
    public void run(String... args) {
        List<Double> batchInsertAverages = new ArrayList<>(); //삽입 평균 시간을 저장할 리스트
        List<Double> batchFetchAverages = new ArrayList<>(); //조회 평균 시간을 저장할 리스트
        List<Double> batchDeleteAverages = new ArrayList<>(); //삭제 평균 시간을 저장할 리스트
        ...
        for (int batch = 0; batch < COUNT / BATCH_SIZE; batch++) {
            log.info("배치 작동중 {} of {}", batch + 1, COUNT / BATCH_SIZE); //현재 배치 번호를 로그로 출력
            List<Long> insertTimes = performBatchOperations(batch, "INSERT"); //삽입 작업 수행 및 시간 측정
            List<Long> fetchTimes = performBatchOperations(batch, "SELECT", testDataMapper); //조회 작업 수행 및 시간 측정
            List<Long> deleteTimes = performBatchOperations(batch, "DELETE"); //삭제 작업 수행 및 시간 측정

            batchInsertAverages.add(average(insertTimes)); //삽입 작업의 평균 시간 계산 및 저장
            batchFetchAverages.add(average(fetchTimes)); //조회 작업의 평균 시간 계산 및 저장
            batchDeleteAverages.add(average(deleteTimes)); //삭제 작업의 평균 시간 계산 및 저장

            log.info("배치 실행 종료 {}.", batch + 1);
        }

        logBatchResults(batchInsertAverages, batchFetchAverages, batchDeleteAverages);
        log.info("모든 배치 종료."); //모든 배치 작업 종료 로그 출력
    }
        ...
        for (int i = 1; i <= BATCH_SIZE; i++) {
            ...
            switch (operationType) {
                case "INSERT":
                    ...
                    operationTimes.add(executeUpdateSql(sql, keyIndex, "TestData" + keyIndex, keyIndex)); //삽입 쿼리 실행 및 시간 측정
                    break;
                case "SELECT":
                    sql = "SELECT * FROM test_data WHERE id = ?";
                    operationTimes.add(executeQuerySql(sql, keyIndex, mapper[0])); //조회 쿼리 실행 및 시간 측정
                    break;
                case "DELETE":
                    sql = "DELETE FROM test_data WHERE id = ?";
                    operationTimes.add(executeUpdateSql(sql, keyIndex)); //삭제 쿼리 실행 및 시간 측정
                    break;
            }
        }
    ...
    private void logBatchResults(List<Double> insertAverages, List<Double> fetchAverages, List<Double> deleteAverages) {
        for (int i = 0; i < insertAverages.size(); i++) {
            log.info("Batch {} - 평균 삽입 시간: {}ms, 평균 조회 시간: {}ms, 평균 삭제 시간: {}ms",
                    i + 1, insertAverages.get(i), fetchAverages.get(i), deleteAverages.get(i)); //배치 결과 로그 출력
        }
    }
